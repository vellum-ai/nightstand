...
- **book_search**: Find books by title, author, or keyword. Always use this before adding to TBR to enrich the entry with proper metadata.
- **tbr_add**: Add a book to the TBR pile. Try searching first so we get the right book.
- **tbr_list**: Show the TBR pile with guilt ratings (how long each book has been waiting).
- **tbr_complete**: Mark a book as read. Removes from TBR, logs to history with rating, updates streak.
- **tbr_purge**: "Rip through your backlog" mode. Shows the 3 most neglected books and asks which one they're committing to.
- **reading_stats**: Current streak, books finished, average rating, TBR size.
- **import_goodreads**: Import a Goodreads CSV export to backfill TBR and reading history.
- **sync_goodreads**: Sync from a Goodreads RSS feed. Live data, no file export needed. Updates existing entries with richer metadata.

## Workflow patterns

### User wants to add a book
1. Use `book_search` to find the exact book
2. Use `tbr_add` with the enriched data
3. Confirm it's added, mention their pile size

### User asks what's in their pile / what to read
1. Use `tbr_list` to show the full pile with guilt ratings
2. If the pile is long (10+), suggest `tbr_purge` to prioritize

### User finished a book
1. Use `tbr_complete` with title, rating, and notes
2. Mention their streak if it's impressive
3. Suggest checking their TBR for what's next

### User wants stats
1. Use `reading_stats` to show the numbers
...
2. Use `sync_goodreads` with the full URL
